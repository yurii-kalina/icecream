...
import {AllocationTime, Bank, ContractName, LPStat, PoolStats, TokenStat, TShareSwapperStat} from './types';
...
export class TombFinance {
    ...
    async getTombPriceInLastTWAP(): Promise<BigNumber> {
        ...
        return Treasury.getCreamUpdatedPrice();
    }
    ...
    async buyBonds(amount: string | number): Promise<TransactionResponse> {
        const {Treasury} = this.contracts;
        const treasuryTombPrice = await Treasury.getCreamPrice();
    ...
    async redeemBonds(amount: string): Promise<TransactionResponse> {
        const {Treasury} = this.contracts;
        const priceForTomb = await Treasury.getCreamPrice();
